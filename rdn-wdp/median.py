...
# @Float(label="Min Median XY sigma", value=2) misigma
# @Float(label="Max Median XY sigma", value=2) masigma
...
for msigma in range(int(round(misigma)), int(round(masigma)) + 1):
    for msigmaz in [msigma, msigma / 2]:
        ...
        mask.setTitle("mask")    
        folder = str(folder)
        basename = os.path.join(folder, method_threshold + "-" + str(msigma) + "_" + str(msigmaz))
        hdf5 = basename + ".hdf5"
        for image in [median, mask]:
